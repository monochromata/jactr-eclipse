package org.jactr.eclipse.tools.shell;
...
import org.jactr.tools.shell.IInterpreterManipulator;
...
public class EclipseManipulator implements IInterpreterManipulator
{
  ...
  private void importCommands(Interpreter interpreter)
  {
    for (IConfigurationElement config : getExtensionInfo(
        "org.jactr.eclipse.tools.shell.commands", "package"))
    {
  ...
  {
    for (IConfigurationElement config : getExtensionInfo(
        "org.jactr.eclipse.tools.shell.commands", "source"))
    {
      ...
      String type)
  {
    ...
    IExtensionPoint exp = registry
        .getExtensionPoint("org.jactr.eclipse.tools.shell.commands");
